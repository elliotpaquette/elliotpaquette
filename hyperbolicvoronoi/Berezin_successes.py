...
def generateKMatrix(s,points,weights):
    ...
    for i in range(N):
        for j in range(N):
            ...
            A[i,j] = np.power(1-u.dot(u),x)*np.sqrt(weights[i]*weights[j])/np.pi;
    return(A);
...
if __name__ == '__main__':
	...
	description='Sample the Stadium or Permuted Stadium + noise, which is n x n + n^{-gamma}G')
	...
	help='s')
    	parser.add_argument(
	'--log', type=argparse.FileType('a+'), default=sys.stdout,
	help='the file where the gap should be written '
	...
	s = args.s;
        ...
        K = generateKMatrix(s,points,weights)     
        choices = sampleKPP(s*K)
        
        args.log.write('%s\n' % len(choices))
        
	
#	for i in range(len(evals)):
#		args.log.write('%.6f %.6f\n' % (evals.real[i], evals[i].imag));
