...
#generate n uniform points from a euclidean ball of radius r
...
#compute the radius of the ball of the circumcircle through three points
def HCR(p1,p2,p3):
    a = abs(p1 - p2)
    b = abs(p2 - p3)
    c = abs(p3 - p1)
...
#Check if the hyperbolic circumcircle containing p is finite
def checkFinite(p):
    p1=p[0]
    p2=p[1]
    p3=p[2]
...
#Compute the mobius map phi_a(p) of a point p, with phi_a the map interchanging a and 0
def MobiusMap(a,z):
    zz = z[0]+z[1]*1.0j
    az = a[0]+a[1]*1.0j 
    x = (az - zz)/(1-(az.conjugate())*zz)
    return np.array([x.real,x.imag])

#Find the hyperbolic angle at x between y and z
def getHAngle(x,y,z):
    u = MobiusMap(x,y);
    v = MobiusMap(x,z);
    return np.arccos(u.dot(v)/np.sqrt(u.dot(u)*v.dot(v)))

#Generate the Berezin matrix discretized on points with weights s
def generateKMatrix(s,points,weights):
    nrm = (spsp.gamma(s*0.5)**2.)/(spsp.gamma(s*0.5+0.5)**2.) 
    x=(1+s)/2
    N = len(points)
    A = np.zeros((N,N))
    for i in range(N):
        for j in range(N):
            u = MobiusMap(points[i],points[j])
            A[i,j] = np.power(1-u.dot(u),x)*np.sqrt(weights[i]*weights[j])/nrm;
    return(A);

#Generate the DPP with matrix A using the Poulson algorithm
def sampleKPP(A):
    N = A.shape[0]
    sample = []
    for j in range(N):
        if npr.rand() < A[j, j]:  # Bernoulli(A_jj)
            sample.append(j)
        else:
            A[j, j] -= 1  
        A[j+1:, j] /= A[j, j]
...
if __name__ == '__main__':
	...
	description='Sample Berezin Voronoi on a ball of radius 0.99 downsampled from N points')
	...
	help='s')
    	parser.add_argument(
	'oprefix', type=str, default="BerezinVoronoi",
	...
	N = args.N;
	s = args.s;
        points = HyperbolicPoints(N,0.99)
        tri = Delaunay(points);
        weights = np.zeros(N);
...
#print(len(tri.simplices))
        ...
        for i in range(0,len(tri.simplices)):
          if checkFinite(points[tri.simplices[i]]):
            p1 = points[tri.simplices[i][0]]
            p2 = points[tri.simplices[i][1]]
            p3 = points[tri.simplices[i][2]]
            area = np.pi - (getHAngle(p1,p2,p3) + getHAngle(p2,p3,p1) +getHAngle(p3,p1,p2))
            weights[tri.simplices[i][0]] += area
            weights[tri.simplices[i][1]] += area
            weights[tri.simplices[i][2]] += area

#print("Completed computing weights.")

        K = generateKMatrix(s,points,weights/(6.*np.pi))
#factor of 2pi differing from standard area measure.
#combinatorial factor of 3 as triangles are chosen three times.

        choices = sampleKPP(K)
#print("The number of points sampled is: " + str(len(choices)))
        dpp = points[choices]

        tri = Delaunay(dpp);

#generate full picture
        fname = args.oprefix + "_s" + str(s) + "_N" + str(N) + "_Delaunay.svg"

        sys.stdout = open(fname, "w")

        initSVG();
        ind, indp = tri.vertex_neighbor_vertices;
#for i in range(0,10):#range(0,len(dpp)):
        for i in range(0,len(dpp)):#range(0,lep)):
            drawCell( dpp[i], dpp[indp[ind[i]:ind[i+1]]])

        for i in range(0,len(tri.simplices)):
            if checkFinite(dpp[tri.simplices[i]]):
                drawTri(dpp[tri.simplices[i]]);
        closeSVG();

        sys.stdout.close()

#generate Voronoi + points picture
        fname = args.oprefix + "_s" + str(s) + "_N" + str(N) + "_Voronoi.svg"
...
#for i in range(0,10):#range(0,len(dpp)):
        ...
        for i in range(len(dpp)):
            print '<circle cx="{}" cy="{}" r="0.01" fill="blue" stroke="blue" stroke-width="0.001" />\n'.format(dpp[i,0], dpp[i,1]);
        closeSVG();

        #generate Voronoi + points picture
        fname = args.oprefix + "_s" + str(s) + "_N" + str(N) + "_Points.svg"
